import { ArrowRight, Phone, ShieldCheck, WhatsappLogo } from "@phosphor-icons/react/dist/ssr";
...
export function Footer() {
  return <footer className="footer"><div className="container footer-top"><div><Link className="brand-logo-chip footer-logo" href="/" aria-label={`${company.displayName} home`}><Image src="/hbl-001-supreme-clinic-demo/torvent-logo.png" alt="TORVENT" width="2172" height="724"/></Link><p>Responsible AI systems for the work your business does every day.</p><span className="footer-trust"><ShieldCheck/> Human accountability by design</span></div><div className="footer-contact"><small>TALK TO THE FOUNDER</small><b>{company.founder.name}</b><a href={`https://wa.me/${company.whatsappNumber}`} target="_blank" rel="noreferrer"><WhatsappLogo weight="fill"/> {company.phone}</a><a href={`tel:${company.phoneE164}`}><Phone/> Call during business hours</a><p>{company.businessHours}</p></div><div className="footer-place"><small>BUILT FROM</small><b>Hubballi, Karnataka</b><p>Designed for growing teams across <span className="india-word">India</span>.</p></div></div><div className="container footer-grid">{groups.map(([title,links])=><div key={title}><b>{title}</b>{links.map(([label,href])=><Link href={href} key={href}>{label}</Link>)}</div>)}</div><div className="container footer-bottom"><span>© {new Date().getFullYear()} {company.displayName}. All rights reserved.</span><span><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link><Link href="/cookies">Cookies</Link></span></div></footer>;
}

export function CTA() { return <section className="cta-section"><div className="container cta-grid"><div><p className="eyebrow">FIND THE RIGHT FIRST USE CASE</p><h2>Start with the workflow that costs your team the most time.</h2></div><div><p>Tell us where requests wait, information gets re-entered or follow-ups are missed. We will map the opportunity, boundaries and evidence needed for a useful first pilot.</p><Link className="button button-light" href="/contact">Show Us Your Workflow <ArrowRight/></Link></div></div></section>; }

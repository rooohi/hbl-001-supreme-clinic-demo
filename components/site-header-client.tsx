...
const navigation = [
  ...
  { label: "Company", links: [["About","/about"],["Workflow Library","/case-studies"],["Insights","/insights"],["Hubballi","/locations/hubli"],["Contact","/contact"]] },
] as const;
...
export function SiteHeader(){
  ...
  return <><a className="skip-link" href="#main-content">Skip to content</a><header className="site-header"><div className="container nav-inner">
    ...
    <details className="mobile-menu"><summary>Menu</summary><div>{navigation.map(group=><section key={group.label}><b>{group.label}</b>{group.links.map(([label,href])=><Link href={href} key={href} aria-current={isCurrent(pathname,href)?"page":undefined}>{label}</Link>)}</section>)}<Link className="mobile-menu-cta" href="/contact">Show us one workflow <ArrowRight/></Link></div></details>

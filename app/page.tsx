...
import { ArrowDown, ArrowRight, BookOpenText, CalendarCheck, ChartLineUp, CirclesFour, Globe, GraduationCap, Headset, Lifebuoy, MapPin, Pulse, ShieldCheck, Translate, UsersThree } from "@phosphor-icons/react/dist/ssr";
...
const agents = [
  {label:"SALES",title:"AI Sales Agent",copy:"Understands interest, qualifies the opportunity and keeps the next action moving.",boundary:"Commercial decisions follow your rules.",capabilities:["Qualify enquiries","Prioritise follow-ups","Prepare CRM context"],href:"/ai-employees/sales-agent",icon:ChartLineUp},
  {label:"FRONT DESK",title:"AI Receptionist",copy:"Handles multilingual enquiries, gathers context and coordinates permitted bookings.",boundary:"Uncertain requests move to human review.",capabilities:["Answer approved questions","Capture context","Coordinate bookings"],href:"/ai-employees/receptionist",icon:Headset},
  {label:"ADMISSIONS",title:"AI Admission Officer",copy:"Guides families from first question to a complete, usable admission record.",boundary:"Eligibility exceptions remain with the team.",capabilities:["Explain requirements","Collect records","Track next steps"],href:"/ai-employees/admission-officer",icon:GraduationCap},
  {label:"SUPPORT",title:"AI Support Agent",copy:"Resolves repeat requests from approved knowledge and structures every escalation.",boundary:"Low-confidence cases never disappear.",capabilities:["Resolve repeat requests","Structure escalations","Maintain history"],href:"/ai-employees/support-agent",icon:Lifebuoy},
] as const;
...
export default function Home(){return <><Header/><main>
  ...
  <section className="v2-roles" id="solutions"><div className="container">
    ...
    <div className="v2-role-grid">{agents.map(agent=>{const Icon=agent.icon;return <Link href={agent.href} className="v2-role-card" key={agent.title}><div className="v2-role-icon" aria-hidden="true"><Icon weight="duotone"/></div><div className="v2-role-content"><small>{agent.label}</small><h3>{agent.title}</h3><p>{agent.copy}</p><ul>{agent.capabilities.map(capability=><li key={capability}>{capability}</li>)}</ul><span><ShieldCheck weight="duotone"/>{agent.boundary}</span><b>View role <ArrowRight/></b></div></Link>})}</div>
  </div></section>
